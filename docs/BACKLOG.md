...
| 6 Painting workflow (in progress) | Active layer sets canvas and UV map (also from the node editor); toggle paint mode (no channel isolate); brush and colour panels on both the 4.2 brush and 4.3+ asset brush paths; image save and pack policy. Done: 060 and 061, `tests/test_painting.py`. Found: clicking a node in the node editor reports nothing (no update, handler or message bus notification), so a draw callback notices and a timer syncs; a material slot change is no depsgraph update, only a message bus notification, which v2 missed; operators called from a script run no depsgraph handlers until the depsgraph is evaluated | 060, 061, 033, 056 |
