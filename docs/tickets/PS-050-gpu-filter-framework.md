...
- `paint_system.apply_filter(filter_id)` operator: resolves the target
  ...
  selection mask when one exists (PS-091), writes the result with
  `write_pixels` (PS-090, which registers it with Blender's image undo;
  a bare `ed.undo_push` does not restore scripted pixel writes, so the
  operator has no `UNDO` option), saves or packs per PS-056.
