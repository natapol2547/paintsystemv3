...
from ...nodetree.stack_ops import content_input


class PaintSystemFolderLayerNode(PaintSystemLayerNode, Node):
    """A layer whose content is the stack linked into its ``Content Color`` input.

    The content composites over a transparent backdrop. The result then
    ...
    def init(self, context):
        ...
        # Mask stays the last input, below the stacks.
        self.inputs.move(self.inputs.find('Mask'), len(self.inputs) - 1)
